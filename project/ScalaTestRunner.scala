...
object ScalaTestRunner {
	...
	private def invokeScalaTestInSeparateProcess(scalaTestCommand: List[String], logError: String => Unit,
																							 timeout: Int): String = {
	...
	private def scalaTestCommand(testClasses: File, outfile: File, resourceFiles: List[File],
															 gradeOptions: Map[String, String], classpathString: String,
	...
	def scalaTestGrade(gradingReporter: GradingFeedback, classpath: Classpath, testClasses: File, outfile: File,
		...
		val (score, maxScore, feedback, runLog) =
			runScalaTest(classpath, testClasses, outfile, resourceFiles, gradeOptions, gradingReporter.testExecutionFailed,
				instragentPath)
